...
// CSCI 172 - Computer Graphics (Project 6)
...
void mouseMove(int x, int y) //The mouse movement
{
    ...
    //Mouse movement will also move the lighting around
    const double t = glutGet(GLUT_ELAPSED_TIME) / 1000.0; //Speed of lighting spin
    const double spin = t*100.0; //Speed for lighting to turn
    glPushMatrix ();
    GLfloat position[] = { 0.0, 5.0, 3.5, 1.0 };
...
static void display(void)
{
    ...
    gluLookAt(0, 5.0, 30.0, 0.0, 0.0, 0.0, 0.0, 180.0, 100);
    ...
    glCallList(bunny); //Calls bunny model
    glPopMatrix();

    //-- Lighting -- (This version will not be binded with mouse control and lighting will spin around object automatically)
    /*const double t = glutGet(GLUT_ELAPSED_TIME) / 1000.0; //Speed of lighting spin
    const double spin = t*100.0; //Speed for lighting to turn
    glPushMatrix ();
    GLfloat position[] = {0.0, 2.0, 3.5, 1.0 };
    glRotatef (spin, 0.0, 1.0, 0.0);
    glLightfv (GL_LIGHT0, GL_POSITION, position);
    glPopMatrix();*/
...
static void init(void)
{
    ...
    bunny = loadObject("model/bunny.obj"); //Loading Stanford bunny model
...
int main(int argc, char* argv[])
{
    ...
    glutCreateWindow("Project #6 (Bunny)");
    init();
